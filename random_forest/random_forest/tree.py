...
def find_best_split(col_names, rows):
    """Find the best question to ask by iterating over every feature / value and calculating the 
    information gain.
    """
    best_gain = 0
    best_question = None
    ...
    for col in range(num_feats):
        # Extract unique values from the dataset in a given column.
        values = set([row[col] for row in rows])
        for val in values:
            # Create a question, if the value is numeric, then the question is whether input is
            # greater or equal to val. If the value is string, then the question si whether input is
            # equal to val.
...
def build_tree(col_names, rows):
    ...
    The function will return a top level decision node which has multiple branches if the rows are 
    separable. One can use the root node to traverse through the tree and seek leaf nodes.
    """
